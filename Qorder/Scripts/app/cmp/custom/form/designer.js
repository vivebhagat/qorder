    ...
    controller: function ($scope, $sce, $rootScope, $location, $http, defaultService, authService, $compile, $window) {
        ...
        $scope.GetList = function ()
        {
            if ($scope.expanded)
            {
                ...
                _appget($http, {
                    url: api_url + '/api/AuthIntUI/ListDirectory/?Path=' + $scope.parent_link.value + $scope.model.Item2,
                    resp: function (response) { $scope.MainList = response.data; $scope.expanded = true; }
                });
            }
        };
...
var commonTreeSetup = function (app,  tag) {
    ...
    app.component(tag, {
        ...
        template: `<div class="card">
                    ...
                    <h5 class="card-title mb-0">Server Storage </h5></div>
                    <div class ="card-body"><div ><b>Directory </b><input ng-model="Path"  >
                    <button class="btn btn-sm btn-primary" ng-click="SetPath()">Set Path</button></div>
                    <br> {{model.Item2}} <tree-node input="model" parent="{value:'/'}">
                    </tree-node></div></div>`
    });
};

commonTreeSetup(app, 'designer');